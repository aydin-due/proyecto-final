    ...
    if 'username' in session:
        ...
        if request.method == 'POST':
            ...
            if (username not in diccionario_usuarios): #si cambiaron el usuario
                if diccionario_usuarios[user]['email'] == email and diccionario_usuarios[user]['password'] == password: #checar que el correo y contraseña coinciden
                    diccionario_usuarios[username] = diccionario_usuarios.pop(user)
                    del diccionario_usuarios[user]
                    with open('usuarios.json', 'w') as fp:
                        json.dump(diccionario_usuarios, fp)
                    session['username'] = username
                    return redirect('/')
                return render_template('register.html', error='No se puede cambiar el usuario si la contraseña y correo no coindicen con tu cuenta.', username=user)
            else: #si no cambiaron el usuario, cambiar las llaves de éste
